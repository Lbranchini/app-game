...
def start_draft(
    ...
    try:
        return runtime.draft_service.start(
...
def submit_ban(
    ...
    try:
        return runtime.draft_service.submit_ban(draft_id, body.side, body.character_id)
    except DraftError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="draft not found") from exc
...
def submit_pick(
    ...
    try:
        return runtime.draft_service.submit_pick(draft_id, body.side, body.character_id)
    except DraftError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="draft not found") from exc
...
def cancel_draft(
    ...
    try:
        return runtime.draft_service.cancel(draft_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="draft not found") from exc
